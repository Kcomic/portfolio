...
endDate: 2025-12-12
metaDescription: "Rebuilding MongoDB aggregation, index design, and pagination for high-volume campaign reporting — fixing slow queries and memory-heavy operations at scale."
---

import CaseStudySection from "@/components/CaseStudySection.astro";
import ReportingPipeline from "@/components/ReportingPipeline.astro";
...
<figure data-reveal class="my-2 rounded-lg border border-line-soft bg-ink-900/40 p-5 sm:p-6 stage-nodes">
  ...
  <figcaption class="mt-4 text-center font-mono text-xs text-dim">
    Same query, restructured: an index-aligned pipeline reads only what it returns — latency tracks the result size, not how much campaign history has accumulated
  </figcaption>
</figure>
